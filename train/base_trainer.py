...
#         wandb.init(
#             project="Final_Q_learning_cartpole",
#             config=self.config,
...
class StopTrainingOnPatience(BaseCallback):
    def __init__(self, min_improvement: float = 0, patience: int = 10):
        super().__init__()
        self.min_improvement = min_improvement
        self.patience = patience
        self.patience_counter = 0
...
class BaseTrainer:
    def __init__(self, algo_name: str, config: dict, env_id: str, run_name=None):
        ...
        self.run_name = run_name or f"train_{self.algo_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self._init_wandb()
        self.env = self._init_env()
        self.action_noise = self._init_noise()
    ...
    def get_callbacks(self):
        ...
        patience_callback = StopTrainingOnPatience(min_improvement=1.0, patience=10000000)
        self.custom_callback = custom_callback

        return [wandb_callback, custom_callback, patience_callback]
