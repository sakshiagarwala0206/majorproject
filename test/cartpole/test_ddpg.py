...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, required=True, help="Path to DDPG model (.zip)")
    ...
    if args.use_wandb:
        wandb.init(
            ...
            config=vars(config),
            name=f"eval_ddpg_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        )
    ...
    logger.info(f"🔍 Evaluating DDPG controller...")
