...
if __name__ == "__main__":
    ...
    if args.use_wandb:
        wandb.init(
            ...
            config=vars(config),
            name=f"eval_DDPG_{datetime.now():%Y%m%d_%H%M%S}",
        )
    ...
    controller = DRLController(model, "DDPG", env.action_space)

    # Evaluate
    logger.info("Evaluating AssistiveWalkerContinuousEnv with DDPG...")
