...
if __name__ == "__main__":
    ...
    if args.use_wandb:
        wandb.init(
            ...
            config=vars(config),
            name=f"eval_SAC_{datetime.now():%Y%m%d_%H%M%S}",
        )
    ...
    controller = DRLController(model, "SAC", env.action_space)

    # Evaluate
    logger.info("Evaluating AssistiveWalkerContinuousEnv with SAC...")
